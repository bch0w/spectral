...
def rotate_synthetic_sac_files():
    """
    rotate the NEZ output of SPECFEM GLOBE to RTZ using the source receiver
    text file generated from the kaikoura data download function.
    works in place, change function call at bottom of script and call this
    script inside the directory you want to rotate
    :return:
    """
    src_rcv_txt = ("/seis/prj/fwi/bchow/spectral/"
                   ".scratch/kaikoura/source_receiver.txt")
    with open(src_rcv_txt,"r") as f:
        lines = f.readlines()
    for line in lines[1:]:
        stanet, dist, az, baz, flag = line.strip().split(',')
        north = glob.glob("{}.MXN.sem.sac".format(stanet))
        east = glob.glob("{}.MXE.sem.sac".format(stanet))
        if not north or not east:
            ...
            for tr in st:
                # manually changed output directories because I couldn't write
                # into the directory I was reading from
                output_filename = (
                    "/seis/prj/fwi/bchow/spectral/.scratch/"
                    "s20rts_Kaikoura_Wang/{stanet}.MX{comp}.sem.sac").format(
                    stanet=stanet, comp=tr.get_id()[-1]
                )
                tr.write(filename=output_filename, format="SAC"
                )
...
def download_kaikoura_data():
    """
    download data for given GSN stations in SAC format for the Kaikoura eq
    :return:
    """
