...
c = Client("IRIS")
inv = c.get_stations(network=GSN[0][0], station=GSN[0][1].strip(), channel="BH?",
                     level="CHANNEL"
                     )
for tup in GSN[1:]:
    try:
        inv += c.get_stations(network=tup[0], station=tup[1].strip(),
                              channel="BH?", level="CHANNEL")
...
def output_inventory_to_specfem_format(inventory,
    ...
    :return:
    """
    template = "{station:>4}{network:>3} {latitude:6.4f} {longitude:6.4f} {elevation:6.1f} {burial:4.1f}\n"
    oldnet = ''
    oldsta = ''
    i=0
    with open(filename, 'w') as f:
        for net in inv:
            for sta in net:
                for cha in sta:
                    if net.code == oldnet and sta.code == oldsta:
                        continue
                    output = template.format(station=sta.code, network=net.code,
