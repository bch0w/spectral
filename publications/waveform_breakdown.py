...
for sta in sta_list:
    ...
    if len(config.component_list) == 1:
        mgmt.st_obs = mgmt.st_obs.select(component=config.component_list[0])
    mgmt.preprocess()
    mgmt.config.unit_output = "none"
    # mgmt.run_pyflex()
    # mgmt.run_pyadjoint()

    mgmt.plot_wav(show=False, save="KHZ_6-30.png", figsize=(10,4), length_sec=180, 
                  fontsize=14,
